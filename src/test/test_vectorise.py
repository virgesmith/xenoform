...
def test_vectorised_max() -> None:
    assert all(maxv(range(5), range(4, -1, -1)) == [4, 3, 2, 3, 4])  # ty: ignore[invalid-argument-type]
...
def test_vectorised_max_mixed_1d() -> None:
    assert all(maxv(range(5), 3) == [3, 3, 3, 3, 4])  # ty: ignore[invalid-argument-type]


def test_vectorised_max_mixed_2d() -> None:
    a = np.ones((3, 3))
    b = np.array([0, 1, 2])
    assert (maxv(a, b) == np.array([[1, 1, 2]] * 3)).all()  # ty: ignore[unresolved-attribute, invalid-argument-type]
